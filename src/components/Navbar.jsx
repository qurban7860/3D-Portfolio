...
const ContactCard = ({ title, icon, link }) => (
  <a href={link} target="_blank" rel="noopener noreferrer" title={title} className="group">
    <div className="w-8 h-8 sm:w-10 sm:h-10 bg-tertiary rounded-full flex justify-center items-center hover:bg-opacity-80 transition duration-300 ease-in-out transform hover:scale-110 cursor-pointer">
...
const Navbar = () => {
  ...
  return (
    <nav className={`${styles.paddingX} w-full flex items-center py-4 fixed top-0 z-50 bg-primary shadow-md border-b border-white/5`}>
      <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
        ...
        <Link to="/" className="flex items-center gap-2 outline-none" onClick={() => { window.scrollTo(0, 0); setActive(""); }}>
