...
const HiringPoint = ({ icon: Icon, title, index }) => (
  ...
  >
    ...
    <h4 className="text-white text-[11px] sm:text-[12px] font-bold leading-tight truncate">{title}</h4>
  </motion.div>
);

const QuickFact = ({ value, label }) => (
  <div className="flex flex-col items-center px-4 sm:px-6 border-r border-white/10 last:border-0 flex-1 min-w-[80px]">
    <span className="text-white font-black text-base sm:text-lg">{value}</span>
    <span className="text-secondary text-[10px] uppercase tracking-widest text-center">{label}</span>
  </div>
);
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        ...
        >
          <p className="text-secondary text-sm sm:text-base leading-relaxed opacity-80">
        ...
        >
          <div className="premium-glass-card p-6 sm:p-8 w-full max-w-sm flex flex-col gap-5 border-[#915EFF]/20 shadow-[0_0_50px_rgba(145,94,255,0.1)]">
             <h4 className="text-white font-black text-center text-lg">Direct Channel</h4>
             <a href="/contact" className={`${styles.glassButtonPremium} w-full py-4 text-[14px] sm:text-[15px] flex items-center justify-center gap-3 shadow-xl`}>
               📧 Start Consultation
             </a>
             <p className="text-secondary text-[10px] text-center opacity-50 italic">Expect a response within 24 hours.</p>
          </div>
        </motion.div>
      </div>
    </div>
